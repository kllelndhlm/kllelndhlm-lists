...
def statistics():
    sql = "SELECT DISTINCT list_name FROM list"
    result = db.session.execute(sql)
    distinct_list = result.fetchall()
    sql = "SELECT COUNT(DISTINCT list_name) FROM list"
    result = db.session.execute(sql)
    count = result.fetchone()[0]
    sql = "SELECT artist, COUNT(artist) AS artist_occurence FROM list GROUP BY artist ORDER BY artist_occurence DESC LIMIT 3"
    result = db.session.execute(sql)
    most_frequent_artist = result.fetchall()
    sql = "SELECT artist, song, COUNT(song) AS song_occurence FROM list GROUP BY artist, song ORDER BY song_occurence DESC LIMIT 3"
    result = db.session.execute(sql)
    most_frequent_song = result.fetchall()
    sql = "SELECT genre, COUNT(genre) AS genre_occurence FROM list GROUP BY genre ORDER BY genre_occurence DESC LIMIT 3"
    result = db.session.execute(sql)
    most_frequent_genre = result.fetchall()
    sql = "SELECT year, COUNT(year) AS year_occurence FROM list GROUP BY year ORDER BY year_occurence DESC LIMIT 3"
    result = db.session.execute(sql)
    most_frequent_year = result.fetchall()
    return render_template("statistics.html", distinct_list = distinct_list, count=count, most_frequent_artist = most_frequent_artist, most_frequent_song = most_frequent_song,  most_frequent_genre =  most_frequent_genre, most_frequent_year = most_frequent_year)

def list_page(page_name):
    sql = "SELECT * FROM list WHERE list_name=:page_name"
    result = db.session.execute(sql, {"page_name":page_name})

    list = result.fetchall()
    page_name = page_name
    return render_template("list_page.html", list = list, page_name = page_name)
